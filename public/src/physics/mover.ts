...
export class Mover {
    ...
    addForce(force) {
        ...
        this.wireframe(d => {
            ...
            this.p.line(this.position.x, this.position.y, direction.x, direction.y)
    ...
    boundaries(offset, str) {
        ...
        } else if (this.position.x > this.p.width - offset) {
            desired = this.p.createVector(-this.maxSpeed, this.velocity.y);
        }
        if (this.position.y < offset) {
            desired = this.p.createVector(this.velocity.x, this.maxSpeed);
        } else if (this.position.y > this.p.height - offset) {
            desired = this.p.createVector(this.velocity.x, -this.maxSpeed);
        }
        if (desired !== null) {
            desired.normalize();
