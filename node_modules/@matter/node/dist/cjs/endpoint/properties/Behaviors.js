...
class Behaviors {
  ...
  initialize() {
    ...
    const initializeBehaviors = (context) => {
      ...
      if (this.#endpoint.env.has(import_protocol.FabricManager)) {
        const fabricIndices = this.#endpoint.env.get(import_protocol.FabricManager).fabrics.map((fabric) => fabric.fabricIndex);
        if (fabricIndices.length > 0) {
          return import_general.MaybePromise.then(
            promise2,
            () => (0, import_FabricScopedDataHandler.limitEndpointAttributeDataToAllowedFabrics)(this.#endpoint, fabricIndices)
          );
        }
      }
      return promise2;
    ...
    let promise = import_OfflineContext.OfflineContext.act(`initialize<${this.#endpoint}>`, initializeBehaviors, { activity });
    const onReady = () => {
  ...
  async close() {
    ...
    await import_OfflineContext.OfflineContext.act(
      `close<${this.#endpoint}>`,
      dispose
  ...
  #activateLate(type) {
    const result = import_OfflineContext.OfflineContext.act(
      ...
      { activity: this.#endpoint.env.get(import_NodeActivity.NodeActivity) }
    );
