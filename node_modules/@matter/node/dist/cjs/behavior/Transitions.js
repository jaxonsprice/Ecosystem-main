...
class Transitions {
  ...
  start(transition) {
    ...
    const attr = {
      ...
      prevStepAt: import_general.Time.nowMs() - (this.#config.stepIntervalMs ?? Transitions.DEFAULT_STEP_INTERVAL_MS),
      distanceLeft
    };
  ...
  get remainingTime() {
    if (this.#config.manageTransitions === false) {
      if (this.#config.transitionEndTimeMs !== void 0) {
        const remaining = this.#config.transitionEndTimeMs - import_general.Time.nowMs();
        if (remaining < 0) {
          return 0;
        }
