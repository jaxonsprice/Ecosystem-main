...
class GeneralCommissioningServer extends import_GeneralCommissioningBehavior.GeneralCommissioningBehavior {
  ...
  async #armFailSafe({ breadcrumb, expiryLengthSeconds }, session) {
    import_protocol.NodeSession.assert(session, "armFailSafe can only be called on a secure session");
  ...
  async commissioningComplete() {
    ...
    import_protocol.SecureSession.assert(session, "commissioningComplete can only be called on a secure session");
