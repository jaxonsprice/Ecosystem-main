...
__export(Model_exports, {
  Model: () => Model,
  StructuralModelError: () => StructuralModelError
});
...
class StructuralModelError extends import_general.ImplementationError {
}
class Model {
  type;
  isSeed;
   ...
   */
  get path() {
    if (this.parent && this.parent.tag !== import_common.ElementTag.Matter) {
      if (this.parent.tag === import_common.ElementTag.Field) {
        return `${this.parent.path}.${(0, import_general.camelize)(this.name)}`;
      }
      if (this.parent.tag === import_common.ElementTag.Cluster) {
        switch (this.tag) {
          case import_common.ElementTag.Attribute:
            return `${this.parent.path}.state.${(0, import_general.camelize)(this.name, false)}`;
          case import_common.ElementTag.Command:
            return `${this.parent.path}.${(0, import_general.camelize)(this.name, false)}`;
          case import_common.ElementTag.Event:
            return `${this.parent.path}.events.${(0, import_general.camelize)(this.name, false)}`;
        }
      }
      const parent = this.parent;
      if (parent.tag !== import_common.ElementTag.Cluster) {
        const parentMetatype = parent?.effectiveMetatype;
        if (parentMetatype === import_common.Metatype.object || parentMetatype === import_common.Metatype.array) {
          return `${parent.path}.${(0, import_general.camelize)(this.name, false)}`;
        }
      }
   ...
   */
  member(key, allowedTags = [import_common.ElementTag.Field, import_common.ElementTag.Attribute]) {
  ...
  require(type, key) {
    ...
    if (member === void 0 && typeof key === "string") {
      member = this.member((0, import_general.camelize)(key, true));
    }
    if (member !== void 0) {
      return member;
    }
  ...
  constructor(definition, ...children) {
    ...
    if (isClone) {
      ...
      if ("description" in definition || "xref" in definition || "details" in definition || "xref" in definition || "errors" in definition || "asOf" in definition || "until" in definition || "matchTo" in definition) {
        this.resource = new import_Resource.Resource(definition);
      }
    }
    if (isClone) {
  ...
  get resource() {
    return this.#resource || (this.#root?.resources || import_Resource.ResourceBundle.default).get(this);
  }
  set resource(resource) {
    if (resource instanceof import_Resource.Resource) {
      this.#resource = resource;
    } else {
      this.#resource = new import_Resource.Resource(resource);
    }
  }
  get description() {
  ...
  get localResource() {
    return this.resource ?? (this.resource = new import_Resource.Resource());
  }
  ...
  [inspect](_depth, options, inspect2) {
    ...
    return `${inspect2(props, options)}`.replace(/^\{/, `${(0, import_general.decamelize)(this.tag)} {`);
  }
}
import_Children.Children.installModelConstructor(Model);
