...
class InteractionClientProvider {
  ...
  async getInteractionClient(address, discoveryOptions) {
    ...
    const nodeStore = isGroupAddress ? void 0 : this.#peers.get(address)?.dataStore;
...
class InteractionClient {
  ...
  maximumPeerResponseTimeMs(expectedProcessingTimeMs) {
  ...
  async processReadRequest(messenger, request, attributeChangeListener) {
    ...
    if (attributeReports.length > 0) {
      logData.push(
        `attributes ${attributeReports.map(({ path, value }) => `${(0, import_types.resolveAttributeName)(path)}=${(0, import_general.serialize)(value)}`).join(", ")}`
      );
    }
  ...
  async processAttributeUpdates(scope, attributeReports, attributeListener) {
    for (const data of attributeReports) {
      ...
      logger.debug(
        `Received attribute update${changed ? "(value changed)" : ""}: ${(0, import_types.resolveAttributeName)({
          endpointId,
          clusterId,
          attributeId
  ...
  async invoke(options) {
    ...
    const invokeResponse = await this.withMessenger(async (messenger) => {
      ...
      const response = await messenger.sendInvokeCommand(
        ...
        expectedProcessingTimeMs ?? (useExtendedFailSafeMessageResponseTimeout ? DEFAULT_MINIMUM_RESPONSE_TIMEOUT_WITH_FAILSAFE_MS : void 0)
      );
      if (response === void 0) {
