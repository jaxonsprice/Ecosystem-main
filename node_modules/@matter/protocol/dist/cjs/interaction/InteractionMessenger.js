...
class IncomingInteractionClientMessenger extends InteractionMessenger {
   ...
   */
  async readAggregateDataReport(chunkListener, expectedSubscriptionIds) {
    let result = void 0;
    let currentEndpointId = void 0;
    let currentClusterId = void 0;
    const currentClusterChunk = new Array();
    let pendingAttributeReports = void 0;
    const handleAttributeReportEntries = (attributeReports, previousPendingAttributeReports) => {
      ...
      if (attributeReports?.length) {
        let lastEndpointId = void 0;
        let lastClusterId = void 0;
        let lastAttributeId = void 0;
        for (let i = attributeReports.length - 1; i >= 0; i--) {
          const attributeReport = attributeReports[i];
          if (attributeReport.attributeData === void 0) {
            break;
          }
          const {
            path: { endpointId, clusterId, attributeId }
          } = attributeReport.attributeData;
          if (lastEndpointId === void 0 && lastClusterId === void 0 && lastAttributeId === void 0) {
            lastEndpointId = endpointId;
            lastClusterId = clusterId;
            lastAttributeId = attributeId;
          }
          if (endpointId === lastEndpointId && clusterId === lastClusterId && attributeId === lastAttributeId) {
    ...
    const processDecodedReport = async (decodedReport, result2) => {
      if (!result2) {
        result2 = decodedReport;
      } else {
        if (!result2.attributeReports) {
          result2.attributeReports = decodedReport.attributeReports;
        } else {
          result2.attributeReports.push(...decodedReport.attributeReports);
        }
        if (Array.isArray(decodedReport.eventReports)) {
          if (!result2.eventReports) {
            result2.eventReports = decodedReport.eventReports;
          } else {
            result2.eventReports.push(...decodedReport.eventReports);
          }
        }
      }
      if (chunkListener !== void 0 && decodedReport.attributeReports) {
    ...
    for await (const report of this.readDataReports()) {
      ...
      result = await processDecodedReport((0, import_DecodedDataReport.DecodedDataReport)(report), result);
    }
    if (pendingAttributeReports?.length && result !== void 0) {
      result = await processDecodedReport(
        (0, import_DecodedDataReport.DecodedDataReport)({
    ...
    if (chunkListener !== void 0 && currentClusterChunk.length > 0) {
      await chunkListener(currentClusterChunk);
      currentClusterChunk.length = 0;
    }
    if (result === void 0) {
  ...
  async readDataReport() {
    ...
    return import_types.TlvDataReport.decode(dataReportMessage.payload);
  }
  /**
...
class InteractionClientMessenger extends IncomingInteractionClientMessenger {
  ...
  async send(messageType, payload, options) {
    try {
      if (this.exchange.channel.closed) {
        throw new import_MessageChannel.ChannelNotConnectedError("The exchange channel is closed. Please connect the device first.");
      }
      return await this.exchange.send(messageType, payload, options);
    } catch (error) {
      if (this.#exchangeProvider.supportsReconnect && (error instanceof import_MessageExchange.RetransmissionLimitReachedError || error instanceof import_MessageChannel.ChannelNotConnectedError) && !options?.multipleMessageInteraction) {
        logger.debug(
  ...
  async readAggregateSubscribeResponse(chunkListener) {
