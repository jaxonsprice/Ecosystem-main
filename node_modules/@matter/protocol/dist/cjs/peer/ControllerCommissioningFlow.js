...
class ControllerCommissioningFlow {
    ...
    for (const step of this.commissioningSteps) {
      ...
      try {
        ...
        if (this.#currentFailSafeEndTime !== void 0) {
          ...
          const timeLeft = Math.floor((this.#currentFailSafeEndTime - import_general.Time.nowMs()) / 1e3);
          if (timeLeft < this.#defaultFailSafeTimeS / 2) {
            logger.info(
              `After Commissioning step ${step.stepNumber}.${step.subStepNumber}: ${step.name} succeeded, ${timeLeft}s left for failsafe timer, re-arming failsafe`
            );
            await this.#armFailsafe();
  ...
  async #armFailsafe(timeS) {
    ...
    this.#currentFailSafeEndTime = import_general.Time.nowMs() + expiryLengthSeconds * 1e3;
    return {
      code: 0 /* Success */,
      breadcrumb: this.lastBreadcrumb
    };
  ...
  get #failSafeTimeLeftS() {
    ...
    return Math.max(0, Math.ceil((this.#currentFailSafeEndTime - import_general.Time.nowMs()) / 1e3));
  }
  async #ensureFailsafeTimerForS(maxProcessingTime) {
  ...
  async #certificates() {
    ...
    const operationalPublicKey = await import_X509Base.X509Base.getPublicKeyFromCsr(this.ca.crypto, certSigningRequest);
  ...
  async #configureNetworkWifi() {
    ...
    if (this.collectedCommissioningData.supportsConcurrentConnection !== false) {
      ...
      const { networkingStatus, wiFiScanResults, debugText } = await networkCommissioningClusterClient.scanNetworks(
        ...
        { expectedProcessingTimeMs: scanMaxTimeSeconds * 1e3 }
      );
    ...
    const connectResult = await networkCommissioningClusterClient.connectNetwork(
      ...
      { expectedProcessingTimeMs: connectMaxTimeSeconds * 1e3 }
    );
  ...
  async #reconnectWithDevice() {
    ...
    const reArmFailsafeInterval = import_general.Time.getPeriodicTimer(
      ...
      this.#defaultFailSafeTimeS / 2 * 1e3,
      () => {
