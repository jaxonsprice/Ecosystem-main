...
class CommandInvokeResponse extends import_DataResponse.DataResponse {
  ...
  async *#processEndpointForWildcard(endpoint, path, commandRef, commandFields) {
    ...
    if (cluster !== void 0) {
      ...
      const command = cluster.type.commands[commandId];
      if (command !== void 0) {
        if (this.session.authorityAt(command.limits.writeLevel, cluster.location) !== import_AccessControl.AccessControl.Authority.Granted || command.limits.timed && !this.session.timed) {
          return;
        }
